...
class Student{
	...
	public List<Student> findStudentByName(String nameStartsWith) {
		char firstAlpha = nameStartsWith.charAt(0);
		String query = "SELECT name from student where name like " + firstAlpha + "%;";
		List<Student> stud = template.query(query);
		for (int i = 0; i < stud.length; i++){
		JSONObject jObj = new JSONObject();
		try {
			jObj.put("student", stud[i]);
		}catch (Exception e)
		{
			e.printstacktrace();
		}
